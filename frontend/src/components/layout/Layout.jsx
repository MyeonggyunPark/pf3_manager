...
export default function Layout() {
  ...
  // Automatically close the notification dropdown and mobile menu when the URL path changes
  // URL 경로가 변경될 때 알림 드롭다운과 모바일 메뉴를 자동으로 닫음
  useEffect(() => {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background text-foreground font-sans transition-colors duration-300">
      ...
      {/* 사이드바 섹션 - 모바일에서 숨김 */}
      <aside className="hidden md:flex w-64 bg-primary dark:bg-card text-white dark:text-card-foreground flex-col shadow-2xl z-20 border-r border-transparent dark:border-border transition-colors duration-300">
        <div className="p-6">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-black tracking-tighter text-white dark:text-primary">
              MS Planer
            </h1>
          </div>
      ...
      {isMobileMenuOpen && (
        <>
          ...
          {/* Mobile Menu */}
          <aside className="fixed inset-y-0 left-0 w-64 bg-primary dark:bg-card text-white dark:text-card-foreground flex flex-col shadow-2xl z-50 md:hidden transform transition-transform duration-300">
            <div className="p-6">
      ...
      <main className="flex-1 overflow-y-auto relative bg-background transition-colors duration-300">
        <header
          className={cn(
            "bg-background/80 backdrop-blur-md border-b border-border sticky top-0 z-10",
            RESPONSIVE_PADDING.md,
            "py-3 md:py-4 flex justify-between items-center gap-4",
          )}
        >
          <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            >
              <LucideIcons.Menu className="w-5 h-5 text-primary" />
            </button>
            ...
            <div className="flex-1 min-w-0">
              <h2
                className={cn(
                  RESPONSIVE_TEXT.xl,
                  "font-bold tracking-tight text-primary truncate",
                )}
              >
                {title}
              </h2>
              <p
                className={cn(
                  RESPONSIVE_TEXT.xs,
                  "text-muted-foreground mt-0.5 md:mt-1 truncate",
                )}
              >
                {desc}
              </p>
          ...
          <div ref={notificationRef} className="flex gap-3 relative">
            ...
            {showNotifications && (
              <div
                className={cn(
                  "absolute top-12 right-0 bg-card text-card-foreground border border-border rounded-xl shadow-xl z-50 animate-in fade-in zoom-in-95 duration-200",
                  RESPONSIVE_DROPDOWN.widthSm,
                )}
              >
        ...
        <div
          className={cn(
            RESPONSIVE_PADDING.md,
            "py-4 md:py-6 lg:py-8 max-w-7xl mx-auto",
          )}
        >
          <Outlet />
        </div>
