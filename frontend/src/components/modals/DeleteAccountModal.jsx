...
export default function DeleteAccountModal({
  ...
  return createPortal(
    <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-2xl border border-white/20 overflow-hidden m-4 animate-in zoom-in-95">
        <div className="p-8 flex flex-col items-center text-center">
          ...
          <div className="text-sm text-center text-slate-500 mb-8">
            정말로 삭제하시겠습니까?
            <p className="mt-1">
              모든 데이터가{" "}
              <span className="text-destructive">영구적으로 삭제</span>되며
              <br />
          ...
          <div className="flex gap-3 w-full">
            ...
            <Button
              onClick={onConfirm}
              disabled={isDeleting}
              className="flex-1 bg-destructive hover:bg-destructive/90 text-white cursor-pointer h-11 font-semibold text-sm"
            >
              {isDeleting ? (
