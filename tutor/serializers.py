...
class StudentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        # Customer number is auto-generated by the model logic
        # 튜터는 ViewSet에서 로그인한 사용자를 기준으로 자동 할당됩니다.
        # 고객 번호(customer_number)는 모델 로직에 의해 자동 생성되므로 읽기 전용입니다.
        read_only_fields = ("tutor", "customer_number")
...
# ==========================================
# 10. Business Profile & Invoice Serializers
# ==========================================
class BusinessProfileSerializer(serializers.ModelSerializer):
    """
    Serializer for Tutor's Business Profile.
    Used in the Settings page to manage invoice sender details.

    튜터의 사업자 프로필을 위한 시리얼라이저.
    설정 페이지에서 영수증 발신자 정보를 관리하는 데 사용됨.
    """

    # Read-only logo URL for frontend display
    # 프론트엔드 표시를 위한 읽기 전용 로고 URL
...
class InvoiceItemSerializer(serializers.ModelSerializer):
    ...
    Validates and handles individual line items in an invoice.

    영수증 상세 항목을 위한 시리얼라이저.
    영수증 내 개별 라인 항목을 검증하고 처리합니다.
    """
...
class InvoiceAdjustmentSerializer(serializers.ModelSerializer):
    """
    Serializer for adjustments (Discounts/Surcharges).
    Applied to the subtotal before VAT calculation.

    조정 항목(할인/추가금)을 위한 시리얼라이저.
    부가세 계산 전 소계(Subtotal)에 적용됩니다.
    """
...
class InvoiceSerializer(serializers.ModelSerializer):
    ...
    Handles nested creation of invoice items and performs precise financial calculations.

    영수증을 위한 시리얼라이저.
    영수증 상세 항목의 중첩 생성을 처리하고 정밀한 재무 계산을 수행합니다.
    """

    # Nested serializers for items and adjustments
    # 항목과 조정 내역을 위한 중첩 시리얼라이저
    items = InvoiceItemSerializer(many=True)
    adjustments = InvoiceAdjustmentSerializer(many=True, required=False)

    # Read-only field for easy identification
    # 식별 편의를 위한 읽기 전용 학생 이름
    student_name = serializers.CharField(source="student.name", read_only=True)
    ...
    def create(self, validated_data):
        """
        Transactional Create for Invoice and Items.
        Standard ModelSerializer does not support nested creates by default.
        Also triggers total recalculation for security.

        영수증 및 항목을 위한 트랜잭션 생성.
        기본 ModelSerializer는 중첩 생성을 기본적으로 지원하지 않습니다.
        또한 보안을 위해 총액 재계산을 트리거합니다.
        """
        # Pop nested data
        # 중첩 데이터 추출
        ...
        for adj_data in adjustments_data:
            InvoiceAdjustment.objects.create(invoice=invoice, **adj_data)

        # Security: Recalculate totals on backend to prevent frontend manipulation
        # 보안: 프론트엔드 조작 방지를 위해 백엔드에서 총액 재계산 수행
